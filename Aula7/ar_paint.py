...
def main():
    ...
    image_sketch2 = zeros([h, w, 3])
    image_over_sketch = zeros([h, w, 3])

    for y in range(h):
        for x in range(w):
            image_sketch[y, x] = [255, 255, 255]
            image_sketch2[y, x] = [255, 255, 255]
            image_over_sketch[y, x] = [255, 255, 255]
    ...
    while True:
        ...
        # indices = np.where(image_sketch2 == 255)
        # image_over_sketch = copy.copy(image_sketch2)
        # image_over_sketch[indices[0], indices[1], :] = image[indices[0], indices[1], :].copy()
        # image_sketch2[indices[0], indices[1], :] = image[indices[0], indices[1], :].copy()
        cv2.imshow(window_name, image_sketch2)
        print(image_sketch2[50, 50, :])

        if flag_video == 0:
            cv2.imshow(window_name, image_sketch)
        else:
            indices = np.where(image_sketch2 == [255, 255, 255])
            image_over_sketch = copy.copy(image_sketch2)
            image_over_sketch[indices[0], indices[1], :] = [255, 0, 0]
            # image_sketch2[indices[0], indices[1], :] = image[indices[0], indices[1], :].copy()
            # # image_sketch2[np.where(image_sketch2 == 255)] = image[np.where(image_sketch2 == 255)]
            # print(image_over_sketch[50, 50, :])
